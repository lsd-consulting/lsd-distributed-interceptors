...
class InteractionGeneratorShould {
    ...
    private static Stream<Arguments> provideInterceptedInteractions() {
        return Stream.of(
                ...
                of(InterceptedInteraction.builder().traceId(TRACE_ID).target("target").serviceName("service").type(RESPONSE).httpStatus("200").body("someValue").elapsedTime(2L).build(), "sync 200 (2 ms) response from Target to Service [#grey]", "{\n  \"body\": \"someValue\"\n}"),
